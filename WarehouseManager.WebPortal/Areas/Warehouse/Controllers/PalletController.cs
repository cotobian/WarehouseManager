...
namespace WarehouseManager.WebPortal.Areas.Warehouse.Controllers
{
    ...
    public class PalletController : BaseController<Pallet>
    {
        ...
        public async Task<JsonResult> GetAllPallet()
        {
            ...
            List<GetPalletVm> list = JsonConvert.DeserializeObject<List<GetPalletVm>>(responseBody).ToList();
        ...
        {
            if (string.IsNullOrEmpty(palletIds)) return NotFound();
            HttpResponseMessage response = await _httpClient.GetAsync(apiUrl + "/PrintPallet/" + palletIds);
            if (!response.IsSuccessStatusCode) return NotFound();
            string responseBody = await response.Content.ReadAsStringAsync();
            string palletNos = JsonConvert.DeserializeObject<string>(responseBody);
            ...
            foreach (string pallet in palletNos.Split(','))
            {
